...
This document describes the *intended* release boundary — it is a specification, not a release record, and never becomes one. Per [Git and Release Strategy](../workflow/02_git_and_release_strategy.md) and `sdd-framework/08_release_process.md`, actual deployment status and history are tracked entirely in `release/` (outside `sdd/`) — this document does not track or claim whether a deployment has occurred, per the framework's own rule that specifications stay unaware of release status. This specification exists to make the release boundary itself unambiguous, independent of when or whether it has been exercised.
