...
## Non-Portable Reporting Sinks (GA4, Amplitude) — Not Providers

Firebase Analytics ("GA4") and Amplitude are explicitly **not** interchangeable Analytics Providers this document governs. Unlike Firestore, PostgreSQL, ClickHouse, BigQuery, Mixpanel, or Segment, neither has a realistic migration path to a self-hosted or provider-swappable backend — each is a terminal, vendor-owned reporting product, not a data store this project owns. Neither is therefore ever expected to become the sole active Provider, and the five-stage Migration Strategy ([Migration Strategy](./05_migration_strategy.md)) never applies to either.

Each may additionally, optionally be enabled as a second (or third), permanent forwarding target — alongside whichever real Provider is currently active — purely for its own out-of-the-box dashboards (GA4's marketing/funnel dashboards; Amplitude's product analytics dashboards, per [ADR-0028](../architecture/decisions/ADR-0028-amplitude-as-a-non-portable-reporting-sink.md)). This is a deliberate, narrowly-scoped exception to the Minimum Abstraction rule above ("simultaneous multi-provider writes as a permanent steady state" is not required by this architecture): the exception is bounded to these named non-portable sinks, for reporting convenience only, and never substitutes for a real Provider being the Event Model's source of truth. The Analytics Service forwards to each sink through its own distinct, explicitly-named path — never through the Provider Interface, and never causing any Feature/Capability/Landing call site to know either sink exists. Each sink receives only the normalized envelope's existing `eventName`/`properties` — no sink-specific event name or taxonomy is introduced (per [Event Catalog](./04_event_catalog.md)'s "no Feature or Capability invents its own `eventName`" rule, which applies to sink integrations identically).

**Amplitude specifically** ([ADR-0028](../architecture/decisions/ADR-0028-amplitude-as-a-non-portable-reporting-sink.md)): event forwarding only. Amplitude's Session Replay capability is explicitly not enabled by this carve-out — turning it on is a distinct decision (data retention, consent/disclosure) requiring its own review before any committed configuration sets its sample rate above `0`. Amplitude's own autocapture (automatic DOM interaction/page-view signals it generates itself, Amplitude-side) is analogous to GA4's own automatic page-view signal — it is not a second project-side taxonomy and is never referenced from any Feature or Capability specification.
