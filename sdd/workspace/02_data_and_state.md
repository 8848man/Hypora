...
|---|---|---|---|
| **Project** | The top-level entity — one per business idea; carries a required Name (identifying label, distinct from any Canvas field) and an optional Description (freeform, distinct from Name and from any Canvas field) | Workspace (authored), Platform API (persisted) | Root of every other concept below |
...
| **Summary** | An AI-synthesized identity narrative of the project (what it is, who it's for, what problem it solves, what value it provides), derived from Canvas only, plus a Summary Lifecycle state | Project (persisted) | **Persisted as its own record, per [ADR-0016](../architecture/decisions/ADR-0016-project-summary-persisted-ai-synthesized-lifecycle.md)** — supersedes this row's prior "never persisted, always derived" definition; narrowed from whole-project to Canvas-only by [ADR-0018](../architecture/decisions/ADR-0018-narrow-project-summary-to-business-canvas-identity-synthesis.md); see [Summary Lifecycle](#summary-lifecycle) below and [Project Summary](./features/05_project_summary.md) |
