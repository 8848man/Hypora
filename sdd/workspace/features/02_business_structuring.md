...
- Offering 3–5 preset answers per question, plus an always-available custom answer — per the [Preset Strategy](./02_1_question_model.md#preset-strategy). On a newly created Project, these presets originate from [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md)'s one-time output when available (tailored presets, or, for the Business Idea question specifically, Thinking Prompts — see Thinking Prompts Presentation below), falling back to static V1 curated presets otherwise, per [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md). This Feature never requests that output itself — [Project Management](./01_project_management.md) triggers the one-time call and hands this Feature whatever result exists by the time each question is shown.
...
- ~~Any AI generation of preset content — V1 presets are static, curated content... Out of scope until V2.~~ **Superseded, not deleted:** the product has reached V2, and [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md) realizes the [Preset Strategy](./02_1_question_model.md#preset-strategy)'s own long-anticipated V2 seam. Static, curated presets remain fully in scope as the permanent fallback whenever AI-tailored presets are unavailable (see Responsibilities above) — this line previously described V1's starting point, which is unchanged in itself; only "AI generation of preset content is categorically out of scope" is no longer true.
- Deciding whether AI-tailored presets are available for a given question — that determination is entirely [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md)'s own Response (or its absence, on failure); this Feature only renders whatever preset source is currently applicable per question, never re-implements that judgment.
- Re-invoking any AI capability to regenerate a question's *initial* onboarding presets — that call happens at most once per Project, owned by [Project Management](./01_project_management.md), per [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md). A user-initiated Regenerate for a single question, once Business Structuring is open, is [Canvas Assistant](../../ai/capabilities/01_canvas_assistant.md)'s existing manual behavior — a separate, already-specified capability, not this onboarding call repeated.
...
- ~~No question's preset content is ever AI-generated in V1.~~ **Superseded, not deleted**, per [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md): a newly created Project's presets may originate from Onboarding Preset Assistant. What remains true from the original criterion: no preset is ever auto-inserted regardless of its source (AI-tailored or static) — see Onboarding Preset Assistant's own [Acceptance Criteria](../../ai/capabilities/07_onboarding_preset_assistant.md#acceptance-criteria).
- A preset sourced from Onboarding Preset Assistant is visually indistinguishable, in its select-or-customize interaction, from a static curated preset — a user is never required to know or care which provider supplied it, per the [Preset Strategy](./02_1_question_model.md#preset-strategy)'s provider-agnostic contract.
- **The preset-selection UI (`ChoiceList`'s select-or-customize interaction) governs every question at every moment, regardless of AI outcome.** Per [ADR-0021](../../architecture/decisions/ADR-0021-always-concrete-onboarding-presets.md), which retired an earlier "Thinking Prompts" non-selectable content type after real usage showed it left a question with zero selectable options: a question either shows AI-tailored presets or falls back to static V1 presets — never a substitute content type, and never fewer than the normal 3–5 selectable options.
...
- ~~V2 (AI Canvas Assistant) becomes a different Preset Provider for these same five questions...~~ **Realized, not future**, per [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md) and [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md): a newly created Project's five questions may now receive AI-tailored presets at creation time, exactly as this section anticipated — the guided flow, the select-or-customize interaction, and the Review step are unchanged; only where the 3–5 suggestions come from changed. [Canvas Assistant](../../ai/capabilities/01_canvas_assistant.md) remains the separate, manual, per-question Preset Provider a user may invoke at any time while the guided flow is open (its own "Ask AI for a suggestion" affordance, unaffected by ADR-0019). Per the [Product Principles](../../context/01_product_vision.md#product-principles)' "AI augmentation, not replacement" rule, an AI-sourced preset — from either source — is still just a suggestion the user may select, edit, or ignore.
