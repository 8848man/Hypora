...
- [Risk Memo Assistant](../../ai/capabilities/02_risk_memo_assistant.md), Contract Version 1.0 (Draft until this Feature is actually implemented) — its own capability specification, never folded into Canvas Assistant's existing contract (their Request/Response shapes diverge from the outset, per the Capability Promotion Rules' mandatory trigger already identified in this architecture's own review history).
