...
**Refs:** → [Features Index](./000_index.md) · [Business Structuring](./02_business_structuring.md) · [Workspace Data & State](../02_data_and_state.md) · [ADR-0004](../../architecture/decisions/ADR-0004-guided-question-flow-for-business-structuring.md) · [ADR-0005](../../architecture/decisions/ADR-0005-korean-first-localization-architecture.md) · [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md) · [Canvas Assistant](../../ai/capabilities/01_canvas_assistant.md) · [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md)
...
*(~~Explicit constraint: V1 uses no AI; presets must be curated, static content in V1, structured so V2 can source them from AI without changing the experience.~~ **Superseded, not deleted:** the product has reached V2, and the seam this constraint asked for is now realized by two concrete AI Preset Providers — see below. V1's static provider remains fully in scope, permanently, as the always-available fallback; it was never meant to be replaced, only optionally supplemented.)*
...
**Replaceable-content contract (the V2 seam, now realized):** a question's preset list is sourced from a **Preset Provider** — a conceptual boundary, not a class or interface design (out of scope for this document per its "no implementation classes" instruction). This project now has **three** coexisting Preset Providers, each serving the identical Input/Output contract below, never competing for the same moment:

| Provider | When it applies | Owning capability |
|---|---|---|
| **Static V1 provider** | Always available; the permanent fallback whenever an AI provider is unavailable, fails, or has not been invoked for a question | None — curated product content, not an AI Capability |
| **Onboarding provider** | A newly created Project's first presentation of each of the five questions, if [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md)'s one-time, automatic call (per [ADR-0019](../../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md)) succeeded with `sufficient` Sufficiency for that question | [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md) |
| **Canvas Assistant provider** | Any later, manual, per-question "Ask AI" / Regenerate invocation while Business Structuring is open — informed by the user's other current answers, not just Name/Description | [Canvas Assistant](../../ai/capabilities/01_canvas_assistant.md) |

Exactly one provider supplies a given question's presets at any one time — never a merge of two providers' output into one list. **Without the guided flow, the Question Model, or the select-or-customize interaction changing at all**, regardless of which provider is currently active. This is what makes the [Product Principles](../../context/01_product_vision.md#product-principles)' "AI augmentation, not replacement" rule concrete for this Feature specifically: AI only ever supplies *suggestions* through the same seam a static list fills — it never gains a different or more privileged interaction path than the one already specified here.

**Every Provider always returns presets — never a substitute content type.** An earlier revision of the Onboarding provider (below) introduced "Thinking Prompts," a non-selectable open-ended-question content type shown in place of presets when Name/Description were judged too thin. Real usage showed this left a question with zero selectable options — indistinguishable, from a user's perspective, from the preset-selection UI itself having been removed. [ADR-0021](../../architecture/decisions/ADR-0021-always-concrete-onboarding-presets.md) retired this content type: every Provider, including Onboarding, now always returns the standard 3–5-option preset shape this document defines — a thin Name/Description is handled by the Onboarding provider inventing plausible, concrete specifics, never by changing the output shape.
...
- **Static V1 provider.** The curated preset list per question exists once per supported language (Korean authored first, per the [Localization Principle](../../context/01_product_vision.md#localization-principle) — English preserves the original Korean meaning rather than being independently authored) — the provider simply selects the current language's list, the same static-content shape already described above, now keyed by language as well as by question.
- **Canvas Assistant provider.** An AI-based provider behind the identical Input/Output contract — receives the current language as part of its input and is responsible for returning presets *already in that language*; it never returns one language for the UI to translate on the fly.
- **Onboarding provider.** Also AI-based and identically localized, but its Input omits "current answers/context" — per [Onboarding Preset Assistant](../../ai/capabilities/07_onboarding_preset_assistant.md#request-contract), no Canvas question has been answered yet at the moment it is invoked; its Input is instead the Project's Name and optional Description (a narrower, creation-time-only substitute for "current answers/context"), still paired with the same current-language field.
- **The provider interface itself remains language-independent** — "language" is just one more input parameter, exactly like "Question ID" or "current answers/context" already were; no separate, parallel per-language provider mechanism exists or is needed. Neither AI provider requires the guided flow, the Question Model, or any UI component to know how localization happened — every provider simply returns options already in the requested language.
