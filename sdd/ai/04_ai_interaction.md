...
| **Automatic invocation** | **Deferred by default; two narrow, named exceptions exist** | The system starting a capability call without an explicit user action (e.g., on field blur, on a timer, on page load) remains out of scope by default. Per Governing Rule 1, merely *showing* an AI affordance is never itself invocation — deferring automatic invocation means no code path may trigger Requesting except a user's explicit action, unless a specific ADR grants a narrow exception. **Exception 1:** [Project Summary Synthesis Assistant](./capabilities/06_project_summary_synthesis_assistant.md)'s `initial_generation` Operation, per [ADR-0017](../architecture/decisions/ADR-0017-automatic-invocation-for-project-summary-initial-generation.md) as amended by [ADR-0018](../architecture/decisions/ADR-0018-narrow-project-summary-to-business-canvas-identity-synthesis.md) — system-triggered exactly once per Project, when the Project's Business Canvas first reaches completion (the domain lifecycle's Structuring → Scoped guard). No other operation of that same capability (its `sync` Operation remains Manual-only) carries this exception. **Exception 2:** [Onboarding Preset Assistant](./capabilities/07_onboarding_preset_assistant.md)'s single Operation, per [ADR-0019](../architecture/decisions/ADR-0019-ai-generated-onboarding-presets-at-project-creation.md) — system-triggered exactly once per Project, immediately after Project creation. Unlike Exception 1, this capability never writes to a Canvas field or any other live content; its output is always presented as offered presets (or Thinking Prompts) through the ordinary select-or-customize interaction, so Governing Rule 2's overwrite protection is never at issue. No other capability, and no other invocation of either of these two, carries an exception. A future capability wanting Automatic Invocation must obtain its own ADR against the same narrow bar ADR-0017 sets, per that ADR's Future Impact — this table is updated to name each such exception individually, never generalized into a third, open-ended "automatic" mode. A capability offering richer seed context when a field is empty (e.g., [Canvas Assistant](./capabilities/01_canvas_assistant.md)'s AI-first Draft Generation) changes what context is available to an invocation, never whether the invocation itself is automatic — that distinction still binds. |
