...
**Every surface in either Application — Landing's header/sections/footer and Workspace's app bar/screens alike — resolves to the same horizontal edges.** Promoted here from being a Landing-only rule once Workspace independently hit the identical defect: Landing's own header once shipped with no width constraint at all ([Landing Design System](../landing/05_design_system.md#shared-layout-container) has that original finding), and Workspace's new app bar shipped the same gap a second time, plus a form narrowed with `max-width` but no centering margin — proof this is a recurring, cross-Application failure mode, not a one-off, and per [Ownership Map](../rules/ownership.md)'s concept-ownership discipline, a concept two areas both need belongs at their shared root (here), never redefined independently by each.

- **One shared container width.** Every surface uses the existing `--max-content-width` token — never an independent, per-surface or per-Application width value.
- **Centering is mandatory, not incidental.** Any element carrying a `max-width` narrower than its parent must also be horizontally centered within that parent (`margin: 0 auto`, or the flex/grid equivalent). A `max-width` applied without centering is a defect against this rule, not a stylistic choice.
- **A header/app bar is not exempt.** It shares the same container width and horizontal gutter as the content beneath it — a user should never see a header's content start or end at a different horizontal position than the screen content immediately below it. Where a bar needs a full-bleed background or border, split it into an outer full-bleed element and an inner width-constrained one (see [Frontend Architecture](../frontend/01_architecture.md#workspace-layout)'s app shell and [Landing Design System](../landing/05_design_system.md#shared-layout-container)'s header for two independent implementations of this same split).
- **A narrower reading width is allowed** (e.g., a form or a centered prose block narrower than the shared container) — but it must itself resolve centered within the shared container, never left-aligned by omission.
- **The horizontal gutter is shared and uniform:** `var(--space-5)`, reduced to `var(--space-3)` at the app's one existing responsive breakpoint (`max-width: 640px`) — applied to every surface in both Applications at that breakpoint, not only to whichever surface first needed it.

Landing's own header-specific detail (its three-region logo/nav/actions centering) stays owned by [Landing Design System](../landing/05_design_system.md#header-navigation-centering) — a Landing-specific instantiation of this shared rule, not a restatement of it.
