...
A single, dedicated persistence-access module is the **only** code permitted to call LocalStorage directly — conceptually, this module is where Platform API's V1 implementation lives inside the shared codebase (see [Application Responsibilities](../context/05_application_responsibilities.md) and [Workspace Data & State](../workspace/02_data_and_state.md)). Every Feature module and every state container calls this module's read/write operations; none call LocalStorage itself. This boundary is what lets a future real Platform API backend replace this module later without requiring every Feature to change (see [Future Expansion Strategy](../context/06_future_expansion_strategy.md)) — realized by [ADR-0025](../architecture/decisions/ADR-0025-account-authentication-and-one-directional-migration.md), which is exactly that replacement for a signed-in caller: this same module now routes to Firestore instead of LocalStorage depending on auth state, with every Feature/state-container call site unchanged.

## Workspace Layout

- **App shell (per [ADR-0026](../architecture/decisions/ADR-0026-workspace-app-shell-for-account-navigation.md)):** a persistent app bar wraps every Workspace screen (Dashboard and every Project view alike) — the single, consolidated home for the Language Switcher and the account/login area ([ADR-0025](../architecture/decisions/ADR-0025-account-authentication-and-one-directional-migration.md)). An App-shell component per Component Ownership above, not owned by any Feature. Bound by [Design System — Shared Layout Container](../design-system/01_design_system.md#shared-layout-container): the bar's content resolves to the same horizontal edges as the screen content beneath it, never an independent width.
- **Dashboard:** a list layout (one row/card per Project, each showing Name and current lifecycle stage) plus the create-Project action — per [Project Management](../workspace/features/01_project_management.md). Renders beneath the app shell.
- **Project view:** a persistent Project-level navigation exposing all four non-linear sections plus Summary, with the current lifecycle stage displayed prominently — per the [Workspace Mental Model Review](../workspace/01_architecture.md#workspace-mental-model-review). Only the content area changes between sections; the navigation shell persists. Also renders beneath the app shell — the two navigation layers are distinct (app-level vs. within-Project) and do not duplicate each other's content.
