...
**Note on Landing and Platform API:** per `10_bootstrap_guide.md` Step 3, a dedicated `sdd/<application>/` directory is created only once that Application has real code. **Landing was promoted 2026-07-17** — its promotion trigger had already fired (real Landing code existed, per `sdd/landing/00_index.md`'s Provenance section) before this directory was created; unlike Workspace's early promotion, this was spec catching up to existing code and a validated redesign direction, not spec leading implementation. Landing's responsibilities summary (Purpose, "Responsible for" bullets, Hard boundary) remains in `sdd/context/05_application_responsibilities.md` per this project's standing convention that no other document restates *what* each Application is for — only its detailed V1 Functional Scope relocated, mirroring Workspace's own precedent exactly. **Platform API as a whole remains unpromoted** — no dedicated `sdd/platform-api/` directory exists, and Workspace's own project persistence is still LocalStorage-only. This is narrower than "no real backend exists": the AI capability specifically **is** a real, deployed backend (Vercel serverless functions, owned by the AI area above, not Platform API's own directory), per [Application Responsibilities](../context/05_application_responsibilities.md#platform-api)'s corrected capability table. Platform API's remaining responsibilities and V1 scope stay recorded in `sdd/context/05_application_responsibilities.md`, owned by the Product/Context area, until Platform API's own promotion trigger fires (a dedicated backend beyond the AI capability, or Authentication/Search/Integrations becoming real).
