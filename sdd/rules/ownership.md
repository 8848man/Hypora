...
Who may modify which files, and the coordination rules for changes spanning areas. Created at project inception, per `10_bootstrap_guide.md` Step 1. Areas originally mapped to the three Applications plus shared governance/context areas ahead of any real codebase; Landing and Workspace (plus the AI and Analytics Platform Services) now have real, deployed implementation, and this map has been kept current with that reality rather than left describing the pre-implementation starting state.
...
| AI | `sdd/ai/**` | Any Feature's own Purpose/Responsibilities/Acceptance Criteria, or the Analytics Platform's own Event Model/Provider Independence/Event Catalog — AI owns the Capability model, LLM Provider Interface/independence, the prompt/context/response Ownership Model, and each Capability Specification under `sdd/ai/capabilities/`, per [ADR-0006](../architecture/decisions/ADR-0006-ai-as-platform-capability.md) through [ADR-0009](../architecture/decisions/ADR-0009-ai-platform-localization-integration.md) |
| Analytics | `sdd/analytics/**` | Any Feature's own Purpose/Responsibilities/Acceptance Criteria, or the AI Platform's own capability contracts — Analytics owns the Event Model, Provider Independence, Event Catalog, Migration Strategy, and the read path/Dashboard boundary ([06_query_and_reporting.md](../analytics/06_query_and_reporting.md)) only, per [ADR-0013](../architecture/decisions/ADR-0013-analytics-provider-independence.md) |
...
| Landing | `sdd/landing/**` (promoted 2026-07-17) | Workspace's or Platform API's owned files |
| Platform API | `sdd/platform-api/**` (promoted 2026-08-07, [ADR-0024](../architecture/decisions/ADR-0024-project-data-durable-server-side-backup.md)) | Landing's or Workspace's owned files; must not dictate Workspace-side UI/navigation behavior |

**Note on Landing and Platform API:** per `10_bootstrap_guide.md` Step 3, a dedicated `sdd/<application>/` directory is created only once that Application has real code. **Landing was promoted 2026-07-17** — its promotion trigger had already fired (real Landing code existed, per `sdd/landing/00_index.md`'s Provenance section) before this directory was created; unlike Workspace's early promotion, this was spec catching up to existing code and a validated redesign direction, not spec leading implementation. Landing's responsibilities summary (Purpose, "Responsible for" bullets, Hard boundary) remains in `sdd/context/05_application_responsibilities.md` per this project's standing convention that no other document restates *what* each Application is for — only its detailed V1 Functional Scope relocated, mirroring Workspace's own precedent exactly. **Platform API was promoted 2026-08-07**, by [ADR-0024](../architecture/decisions/ADR-0024-project-data-durable-server-side-backup.md) — Project data gained a real Firestore backup and Firebase Anonymous Authentication became real, firing the trigger this note previously described as not-yet-fired. The AI capability remains owned by the AI area above, not Platform API's own directory (unchanged). Platform API's own directory (`sdd/platform-api/`) now owns Projects' backup shape and Authentication's two narrow uses; Search and Integrations remain not-implemented and stay recorded in `sdd/context/05_application_responsibilities.md` until each has its own promotion trigger.

**Note on `sdd/landing/improvement/` and `sdd/landing/planning/`:** now that Landing is promoted, both directories fall under the **Landing** area's ownership above (`sdd/landing/**`), not Product/Context — this is a natural consequence of the path-prefix ownership rule, not a re-authorship of their content. Both remain **historical evidence, not specification** — see `sdd/landing/00_index.md`'s "Historical Evidence" section for the permanent rule governing them (frozen; not updated to track the promoted directory's future changes; nothing may be copied out of them without re-deriving it against Hypora's own philosophy).

**Note on `planning/` (repo root, outside `sdd/`):** disposable HTML/prototype artifacts built to validate a Landing design decision live here, never under `sdd/`, since they are explicitly not specifications and are expected to be discarded — see `planning/README.md`. Cited as historical/reference evidence from `sdd/landing/**` (per the promoted directory's own documents), never as a live dependency.
