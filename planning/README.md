...
- `prototype/hypora-navigation-concepts.html` — seven candidate visual treatments for Workspace's project navigation (current/completed/next/future/optional state communication), built to validate the Progressive Navigation review on the `feature/progressive-navigation` branch before any concept is written up as a real specification or ADR. Self-contained, no build step.
